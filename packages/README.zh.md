...
| [`notes/`](notes/README.zh.md) | 模型侧持久工作笔记工具 |
| [`structured-output/`](structured-output/README.zh.md) | 最终回复上的结构化输出契约 |
| [`verifier/`](verifier/README.zh.md) | 独立核验：`ctx.verifier` 裁判服务 + 模型侧 `verify_output` 工具 |
| [`plan/`](plan/README.zh.md) | Plan 协作状态，提供直接进入命令与经评审的退出 |
| [`preset/`](preset/README.zh.md) | 由 preset `cordis.yml` 按会话组装 agent |
| [`guard/`](guard/README.zh.md) | 循环卫生守卫：重复调用提醒、工具调用截止、按会话轮数／token 预算 |
