...
The `guard/` group keeps the agent loop productive by watching for three failure patterns. `repeat-tool-reminder` reminds the model when it repeats the exact same tool call, so a stuck loop stops burning time and tokens. `timeout-policy` times out tool calls that declare a limit, so a hung call returns a clear error instead of stalling the session. `session-budget` caps turns and cumulative tokens per session, so runaway continuation stops at a known cost. All three ship enabled in the `dsh` base bundle; `session-budget` carries runaway-protection defaults.
...
| [`session-budget/`](session-budget/README.md) | Caps turns and cumulative tokens per session, so runaway continuation stops at a known cost |
