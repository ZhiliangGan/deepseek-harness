...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  const seed = (session: Session): JsonSchemaNode | undefined => {
    ...
    // oxlint-disable-next-line typescript/no-deprecated -- Invariant seeds from the complete durable log; migration deferred.
    for (const event of session.snapshotEvents()) schema = applyChecked(schema, event)
