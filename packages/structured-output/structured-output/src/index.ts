...
function finalAssistantText(session: Session, turn: number): string | undefined {
  // oxlint-disable-next-line typescript/no-deprecated -- Tail scan at the stop boundary; projection migration deferred.
...
export class StructuredOutputService extends Service {
  ...
  constructor(ctx: Context, config: Config = {}) {
    ...
    ctx.on('agent/created', ({ agent }) => {
      ...
      // oxlint-disable-next-line typescript/no-deprecated -- Resume dedup scans the durable log; migration deferred.
      const latest = latestArmedContract(agent.session.snapshotEvents())
