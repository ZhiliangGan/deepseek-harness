...
function upsert(seq: number, note: UpsertPayload): SessionEvent {
  return { type: 'notes/change', seq: SessionSeq(seq), time: 0, data: { operation: 'upsert', note } }
}

function deleteEvent(seq: number, id: string): SessionEvent {
  return { type: 'notes/change', seq: SessionSeq(seq), time: 0, data: { operation: 'delete', id } }
}
...
describe('notes stream invariants', () => {
  ...
  it.each([
    ...
    ['carries a non-note upsert payload', [{ type: 'notes/change', seq: SessionSeq(0), time: 0, data: { operation: 'upsert', note: 42 } } as unknown as SessionEvent], /upsert note must be an object/],
    ['carries missing timestamps', [{ type: 'notes/change', seq: SessionSeq(0), time: 0, data: { operation: 'upsert', note: { id: 'a', content: 'x', revision: 1, createdAt: 'soon', updatedAt: 1 } } } as unknown as SessionEvent], /numeric createdAt and updatedAt/],
