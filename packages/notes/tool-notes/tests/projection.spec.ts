...
describe('notes projection unit', () => {
  ...
  it('serves null before any write and the folded note set after writes', async () => {
    ...
    const cold = ctx.sessionProjections.restore({}, empty.snapshotEvents(), SessionLogOffset(0), empty.header, empty.inheritedEventCount)
    ...
    const folded = ctx.sessionProjections.restore(
  ...
  it('exposes no notes key without tool-notes and removes the unit on disposal', async () => {
    ...
    const bareValues = bare.ctx.sessionProjections.restore(
      {}, session.snapshotEvents(), SessionLogOffset(0), session.header, session.inheritedEventCount).snapshot.values
    expect(bareValues.notes).toBeUndefined()
    await bare.ctx.fiber.dispose()

    const { ctx, fiber } = await harness(true)
    expect(ctx.sessionProjections.restore({}, Session.create(SessionId('x')).snapshotEvents(), SessionLogOffset(0), Session.create(SessionId('x')).header, Session.create(SessionId('x')).inheritedEventCount).snapshot.values).toHaveProperty('notes')
    await fiber!.dispose()
    expect(ctx.sessionProjections.restore({}, Session.create(SessionId('x')).snapshotEvents(), SessionLogOffset(0), Session.create(SessionId('x')).header, Session.create(SessionId('x')).inheritedEventCount).snapshot.values.notes).toBeUndefined()
  })
})
