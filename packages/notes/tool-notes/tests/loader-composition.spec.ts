...
function agent(ctx: Context): Agent {
  ...
  const value: Agent = {
    id, options: {}, session, inbox: createInboxStub(),
...
describe('tool-notes real Loader composition through cordis.yml', () => {
  it('applies defaults when the config block is omitted', async () => {
    ...
    const result = await ctx.tools.execute({
      ...
      callId: ToolCallId('notes-default'),
      name: 'write_note',
      arguments: { id: 'decisions', content: 'default caps apply' },
      agent: owner,
    })
    expect(result.isError).toBe(false)
    expect(owner.session.snapshotEvents().findLast(e => e.type === 'notes/change')).toBeDefined()
  }, 30_000)
  ...
  it('honors maxNotes set through cordis.yml end to end', async () => {
    ...
    expect(owner.session.snapshotEvents().filter(e => e.type === 'notes/change')).toHaveLength(1)
  }, 30_000)
  ...
  it('honors maxNoteChars set through cordis.yml end to end', async () => {
    ...
    expect(owner.session.snapshotEvents().some(e => e.type === 'notes/change')).toBe(false)
  }, 30_000)
